...
export async function execute(interaction: any) {
  ...
  if (attachment.contentType && !attachment.contentType.includes('json')) {
    return interaction.reply({ content: 'Please upload a JSON file.', flags: MessageFlags.Ephemeral });
  }
  ...
  try {
    ...
    return interaction.editReply(`✅ Knowledge base updated to v${version.id}.`);
  } catch (error) {
    console.error('Failed to replace knowledge base:', error);
    return interaction.editReply('Failed to process the uploaded JSON.');
  }
}
