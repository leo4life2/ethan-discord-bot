...
export async function execute(interaction: any) {
  ...
  if (versions.length === 0) {
    return interaction.reply({ content: 'No versions yet.', flags: MessageFlags.Ephemeral, allowedMentions: SAFE_ALLOWED_MENTIONS });
  }
  const top = versions.slice(0, 10);
  const lines = top.map(v => `v${v.id} • ${v.updatedAt} • ${v.updatedBy} — ${v.commitMessage || ''}`.trim());
  const content = lines.join('\n');
  return interaction.reply({ content: content, flags: MessageFlags.Ephemeral, allowedMentions: SAFE_ALLOWED_MENTIONS });
}
